...
$(document).ready(function() {
 ...
 window.location = loc;
    ...
    if ($('.price-slider').length > 0) {
        let priceTo = Number($('.price-slider').attr('data-to'))
        let priceFrom = Number($('.price-slider').attr('data-from'))
        let minPriceTotal = Number($('.price-slider').attr('data-min-total'))
        let maxPriceTotal = Number($('.price-slider').attr('data-max-total'))
        $('.price-slider').slider({
            min: minPriceTotal,
            max: maxPriceTotal,
            step: 10,
        ...
        $(".price-slider").on("slide", function(slideEvt) {
          ...
          //let widthBtn = Number($('.filter-accordions').width()) +Number($('.slider-track').offset().left)
          let topBtn = $('.price-range').position().top
          $('._filter_start').css('top', topBtn)
          $('._filter_start').css('display', 'block')
          //$('._filter_start').css('left', widthBtn)
          $("._input_min_price").val(slideEvt.value[0]);
          $("._input_max_price").val(slideEvt.value[1]);
        });
    }
...
if($('#clients-carousel').length>0){
   $("#clients-carousel").carouFredSel({
    ...
    $(".slider").slick({
        ...
        responsive:[{breakpoint:768,settings:{arrows:true}},{breakpoint:480,settings:{arrows:true}}]});
...
if($('#write-review-text').length>0){
    ...
    if ($('.star').length > 0) {
        $('.star').raty({
            starOff: "http://ladybrands/themes/ladybrands/assets/images/star-on.png",
            starOn:  "http://ladybrands/themes/ladybrands/assets/images/star-off.png",
            score: function() {
